...
export const ExploreJoinedCollabs = ({setActiveTab}) => {
  ...
  const fetchInitialCollabs = async (initialLimitRecords) => {
    try {
      ...
      if (res?.data?.status === 'success') {
        const collabReferences = removeDraftCollabs(res?.data?.data?.collabs, user?.userId);
      ...
      {!loading ? (
        ...
        >
          <CollabsMapping
            ...
            emptyStateText='You are not a member of any Collaborations yet. Apply to a Collab to become a member'
            buttonText='Find Collabs to Join'
          />
        </InfiniteScroll>
      ) : (
        <CollabsLoadingGrid />
