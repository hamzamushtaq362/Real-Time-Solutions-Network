...
export const ExploreMyCollabs = () => {
  ...
  const fetchInitialCollabs = async (initialLimitRecords, userId) => {
    try {
      setLoading(true);
      ...
      const res = await reFetchTokenExpire(getCollabs, fetchRefreshToken);
    ...
    try {
      const f1 = async () => {
        return await axios.get(
          `${BASE_URL}/api/v1/recommender/collabs?type=loggedInUser&page=${
            currentPage + 1
          }&limit=${limitRecords}`,
        );
      };
  ...
  useEffect(() => {
    if (windowSize?.width) {
      ...
      fetchInitialCollabs(records, user?.userId);
    }
  }, [windowSize]);
      ...
      {!loading ? (
        ...
        >
          <CollabsMapping
            ...
            collabsType="created"
            emptyStateText="Create Your First Collab"
          />
        </InfiniteScroll>
      ) : (
        <CollabsLoadingGrid />
